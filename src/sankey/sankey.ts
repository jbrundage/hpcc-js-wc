...
export function SankeyChart(
    ...
    const link = d3.select(linksG).selectAll("g").data(links).join("g").style("mix-blend-mode", linkMixBlendMode);
    const gradiantID = ({ source, target }) => `${endcodeID(source.id)}-${endcodeID(target.id)}`;
    link.selectAll("linearGradient")
        .data((d) => (linkColor === "source-target" ? [d] : []), gradiantID)
        .join("linearGradient")
    ...
    link.selectAll("path")
        .data(
            (d) => [d],
            (d) => `${endcodeID(d.source.id)}-${endcodeID(d.target.id)}`
        )
        .join("path")
        .attr("d", linkPath)
